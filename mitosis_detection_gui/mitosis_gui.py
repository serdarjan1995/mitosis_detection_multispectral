...
class Mitosis_GUI(Frame):           
    ...
    def initUI(self):
        ...
        self.csv_browse_btn.grid(row=4, column=2,sticky=tk.W)
        
        
        self.run_btn = Button(self.upper_frame, text="Run Model",command=self.run_btn_func)
        self.run_btn.grid(row=6, column=0, sticky=tk.N)
    ...
    def load_files(self):
        if self.chck_box_regex_val.get() == 0:
            ...
            tk.messagebox.showerror("Error", str_message)
    ...
    def generate_di(self,files_list):
        ...
        if len(self.images)!=10:
            tk.messagebox.showerror("Error", "Please check images directory.\
    ...
    def detect_mitosis(self):
        ...
        try:
            ...
            try:
                ...
                #filtering
                kernel_size = (11,11)
                kernel = np.ones(kernel_size,np.float32)/58
                ...
                if image_mask is not None:
                    _, mask_contours, _ = cv2.findContours(image_mask,cv2.RETR_TREE,
                                                           cv2.CHAIN_APPROX_SIMPLE)
                    print('mask contour:',len(mask_contours))
                    print('mask avg:',np.mean(image_mask))
