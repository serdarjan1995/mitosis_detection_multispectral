...
#            continue
        ...
        for (num,bbox) in enumerate(boundingBoxes):            
            ...
            mitosis = []
            for im in images:
                mitosis.append(im[y:y+w, x:x+h])
                
            mitosis_pattern = np.empty((mitosis[0].shape[0],mitosis[0].shape[1],10))
            ...
            np.save(PATH_PATTERNS+hpf_number_csv_name[:-4]+str(num), mitosis_pattern)
        break
    break
